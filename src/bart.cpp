...
void bart::setdata_without_y(size_t p, size_t n, double *x, size_t numcut)
{
  int* nc = new int[p];
  for(size_t i=0; i<p; ++i) nc[i]=numcut;
  this->setdata_without_y(p, n, x, nc);
  delete [] nc;
}

void bart::setdata_without_y(size_t p, size_t n, double *x, int *nc)
{
   this->p=p; this->n=n; this->x=x;
   if(xi.size()==0) makexinfo(p,n,&x[0],xi,nc);
